...
SELECT DISTINCT SERIES FROM atp_matches_2020;

INSERT INTO pitch (court,surface)
SELECT DISTINCT court, surface FROM atp_matches_2020;

INSERT INTO round (name)
SELECT DISTINCT round FROM atp_matches_2020;

INSERT INTO tournament (sId, name, location)
SELECT DISTINCT series.sId, A.tournament, A.location FROM atp_matches_2020 as A, series
...
SELECT DISTINCT str_to_date(a.match_date,'%d.%m.%Y'), a.l1, a.l2,a.l3,a.l4,a.l5,a.w1,a.w2,a.w3,a.w4,a.w5, a.wrank, a.lrank, a.lpts, a.wpts, a.best_of, tournament.tId, round.rId, winner.pId, loser.pId, pitch.piId  
FROM atp_matches_2020 AS a, series, tournament, round, player AS winner, player AS loser, pitch
